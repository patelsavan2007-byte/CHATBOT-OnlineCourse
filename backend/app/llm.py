...
class LLMStatus:
    ...
    LLM = "llm"                   # answer generated by the Gemini API
...
class LLMClient:
    """Generates answers with a Gemini model and a deterministic fallback."""
    ...
    def generate(
        ...
        *,
        ...
        The API is only used when a key and a candidate model are available;
        otherwise the deterministic local fallback is used.
        """
        ...
        if self._quota_exhausted:
            logger.warning("Gemini quota already known to be exhausted; using local fallback")
            return self._fallback_generate(prompt, context_chunks, conflicts), LLMStatus.QUOTA

        if time.time() < self._cooldown_until:
            logger.info(
                "Gemini API in cooldown (%.0fs left); reusing last status %s",
                self._cooldown_until - time.time(), self._last_status,
            )
        ...
        for model in list(self._working_models):
            try:
                ...
                self._last_status = LLMStatus.LLM
                return text, LLMStatus.LLM
        ...
        logger.warning("All Gemini models failed (%s); entering cooldown", status)
